...
\begin{center}
	\begin{minipage}{0.35\textwidth}
		\includegraphics[width=\textwidth,trim={0 55 0 60},clip]{burgers_temp_ader.png}
	\end{minipage}\,
	\begin{minipage}{0.35\textwidth}
...
\begin{frame}{DeC as RK for ODEs}
	...
	\begin{tikzpicture}[scale=0.8]
			...
			\foreach\l[count=\y] in {M,2,1,0}
			{
				...
				{
					\only<3->{\ifthenelse{\y=4 \OR \x=1}{\fill (\x,-\y) circle (1mm) node[anchor=south west] {$\bc^{\l,(\z)}$};}{\fill[red] (\x,-\y) circle (1mm) node[anchor=south west] {$\bc^{\l,(\z)}$};}}
					\only<2>{\fill (\x,-\y) circle (1mm) node[anchor=south west] {$\bc^{\l,(\z)}$};}
				}
			}
			\only<4->{\fill (5,-3) circle (1mm) node[anchor=south west] {$\bc^{1,(P)}$};}
			\only<4->{\fill (5,-2) circle (1mm) node[anchor=south west] {$\bc^{2,(P)}$};}
